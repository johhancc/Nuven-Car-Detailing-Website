<div align="center">
  <a href="./README.md"><strong>🇺🇸 Read in English</strong></a> | **🇪🇸 Español**
</div>

# Nuven | Plataforma SaaS Premium para Detailing Automotriz

[![React](https://img.shields.io/badge/React-18-blue?style=for-the-badge)](https://reactjs.org/)
[![Supabase](https://img.shields.io/badge/Supabase-Backend-green?style=for-the-badge)](https://supabase.com/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind-Styling-cyan?style=for-the-badge)](https://tailwindcss.com/)
[![Status](https://img.shields.io/badge/Status-Production%20Ready-success?style=for-the-badge)]()

> **Una Solución SaaS Llave en Mano de Alto Rendimiento para la Industria del Detailing.**
> *Combina estética de lujo con un potente CRM y Motor de Reservas.*

---

## 📸 Galería de Funcionalidades

### 🛒 Flujo de Reserva
![Demo de Reserva](./capturas/booking-flow-opt.gif)

### 📊 Dashboard
![Demo del Dashboard](./capturas/dashboard-opt.gif)

### 📊 Panel de Administración
![Demo del Dashboard de Admin](./capturas/adminDashboard-opt.gif)

### 📍 Rastreador en Tiempo Real
![Demo del Rastreador en Tiempo Real](./capturas/realtimetracker-opt.gif)
<br>
<blockquote class="imgur-embed-pub" lang="en" data-id="a/YSdn7DR" data-context="false" ><a href="//imgur.com/a/YSdn7DR">View post on imgur.com</a></blockquote><script async src="//s.imgur.com/min/embed.js" charset="utf-8"></script>

---

## �💼 Resumen Ejecutivo (¿Por qué comprar esto?)

**Nuven** no es solo una plantilla; es una plataforma **SaaS (Software como Servicio)** completamente funcional y lista para producción, diseñada para modernizar negocios de detailing automotriz.

Construida con una filosofía "Mobile-First" y "Centrada en el Usuario", resuelve los tres mayores problemas de los talleres de detailing:
1.  **Fricción en Reservas**: Reemplaza las llamadas manuales con una experiencia de compra fluida tipo Amazon.
2.  **Confianza del Cliente**: Ofrece actualizaciones de estado en tiempo real ("Tu auto está siendo lavado", "Listo para recoger") igual que Uber o Domino's.
3.  **Caos Operativo**: Centraliza todos los pedidos, clientes y finanzas en un robusto Panel de Administración.

**Perfecto para:**
- Desarrolladores que buscan un proyecto de alto valor para revender a negocios locales.
- Emprendedores que lanzan un marketplace tipo "Uber para Detailing".
- Talleres de detailing existentes que quieren dominar su mercado local con tecnología superior.

---

## ✨ Puntos Clave de Venta

### 1. 🎨 UI/UX de Clase Mundial (El Factor "Wow")
- **Diseño Glassmorphism**: Interfaces modernas y translúcidas que transmiten calidad y confianza.
- **Animaciones de Alta Gama**: Potenciadas por **GSAP** y **Framer Motion**. Los elementos flotan, se desvanecen e interactúan suavemente (Botones magnéticos, revelaciones al hacer scroll).
- **Modo Oscuro/Claro**: Totalmente compatible, respetando la preferencia del usuario automáticamente.

### 2. 🤖 Ecosistema de Automatización Completo
- **Motor de Reservas Inteligente**: Previene la doble reserva, maneja la lógica de tamaño de vehículos y calcula tiempos estimados de finalización.
- **Seguimiento en Tiempo Real**: Los clientes reciben un enlace único para ver el progreso de su auto en vivo (Etapas: Recepción -> Lavado -> Pulido -> Control de Calidad -> Listo).
- **Notificaciones Automatizadas**: Actualizaciones por correo electrónico vía integración con Resend para cada cambio de estado.

### 3. 🛠️ CRM Administrativo Potente
- **Vistas Kanban y Lista**: Gestiona el flujo de trabajo eficientemente. Arrastra y suelta reservas entre etapas.
- **Analíticas Financieras**: Gráficos visuales de ingresos, tipos de vehículos y horas pico.
- **Perfiles de Clientes**: Historial detallado de cada cliente, sus vehículos y gasto total (LTV).
- **Acceso Basado en Roles**: Roles seguros de Administrador vs. Usuario.

### 4. 🌍 Listo para Escalar
- **Multilingüe (i18n)**: Cambio instantáneo entre **Inglés** y **Español** (Español). Fácil de agregar más.
- **Backend Serverless**: Construido sobre **Supabase**. Cero mantenimiento de servidores, escala infinitamente y costo extremadamente bajo (Capa gratuita maneja 50k+ MAU).
- **Edge Functions**: Pagos seguros y lógica compleja ejecutada en el borde (edge) para velocidad global.

---

## 🏷️ Opciones de Entrega y Personalización

La demo en vivo representa exactamente lo que recibirás. Tienes dos opciones de compra:

### 1. Entrega Estándar (Tal Cual)
- **Precio: $5,999 USD** *(Negociable)*
- Recibes el código fuente completo exactamente como se ve en la demo alojada.
- Perfecto para desarrolladores o agencias que quieren encargarse de la personalización ellos mismos.

### 2. Servicio Llave en Mano (Configuración Personalizada)
- **Tarifa Adicional: +$1,000 USD**
- **Yo me encargo de todo el rebranding por ti.**
- Incluye: Actualización de todos los datos de contacto, logotipos, esquemas de color, traducciones i18n y tablas de precios para que coincidan con tu negocio específico.
- Resultado: Una plataforma 100% lista para lanzar, entregada a ti en 3-5 días.

---

## 💰 Oportunidades de Monetización
*Cómo puedes ganar dinero con este código:*

1.  **Suscripción SaaS**: Vende este software a talleres de detailing por **$99-$299/mes**.
2.  **Reventa Marca Blanca**: Rebrandear y vender la licencia del código fuente a agencias por **$3k-$5k**.
3.  **Modelo de Marketplace**: Lanza una plataforma de reservas para toda la ciudad y cobra una **comisión del 10-20%** por cada lavado.

---

## 🏗️ Stack Tecnológico (Moderno y Sin Mantenimiento)

Construido bajo la filosofía **T3 Stack** para máxima seguridad de tipos y productividad.

- **Frontend**: React 18 + Vite (Tiempos de carga ultrarrápidos)
- **Estilo**: Tailwind CSS + Sistema de Diseño Personalizado
- **Backend (BaaS)**: Supabase (PostgreSQL, Auth, Storage, Realtime)
- **Gestión de Estado**: Zustand + React Query (TanStack)
- **Animación**: GSAP (Estándar Pro) + Framer Motion
- **Formularios**: React Hook Form + Zod (Validación)
- **Pagos**: Listo para integración con Stripe / MercadoPago

---

## 🤝 Soporte y Entrega

Incluido con la venta:
- **Código Fuente Completo**: 100% de propiedad, sin archivos encriptados.
- **Esquema de Base de Datos**: Scripts SQL para replicar el backend instantáneamente.
- **Activos**: Todos los íconos, imágenes y tokens de diseño.
- **Documentación**: Guía detallada sobre cómo desplegar.

---

## 📞 Contacto y Consultas

¿Listo para comenzar? ¿Tienes preguntas sobre el precio o la tecnología?

- **Discord**: `johan123`
- **Email**: `jxhhxn01@gmail.com`
